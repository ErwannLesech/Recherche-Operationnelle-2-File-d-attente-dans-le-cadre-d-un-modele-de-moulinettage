...
class MG1Queue(BaseQueueModel):
    ...
    def compute_theoretical_metrics(self) -> QueueMetrics:
        ...
        rho = self.rho
        if rho >= 1:
            raise ValueError("Le système est instable (ρ >= 1).")

        # Calcul des métriques théoriques
        C_sq = self.C_squared
        Lq = (rho ** 2) * (1 + C_sq) / (2 * (1 - rho)) if rho < 1 else float('inf')
        ...
        return QueueMetrics(
            ...
            P0=1 - rho,
            Pk=0.0,
            lambda_eff=self.lambda_rate,
            throughput=self.lambda_rate
        )
